...
def get_preprocess_for_dino() -> Compose:
    """
    Get the preprocessing steps for the DinoModel.
    ...
    """
    net_h, net_w = VIT_PATCH_SIZE * MULT_X, VIT_PATCH_SIZE * MULT_Y
    print(f"Using H: {net_h}, W: {net_w}")

    # mean and std are calculated using dataset's pics
    mean = 0.320766
    std = 0.162982
    ...
    transform_func = Compose(
        [
            ...
            NormalizeImage(mean=[mean] * 3, std=[std] * 3),
            PrepareForNet(),
        ]
    )
